...
static bool intel_mid_get_vbt_data(struct drm_psb_private *dev_priv)
{
	...
	case 0x20:
		...
		((struct gct_r20_panel_desc *)panel_desc)->panel_mode.mode ?\
			MDFLD_DSI_ENCODER_DPI : MDFLD_DSI_ENCODER_DBI;
#else
		strncpy(panel_name, fw_panel_info[DEFAULT_PANEL_ID].name, PANEL_NAME_MAX_LEN);
		mipi_mode = fw_panel_info[DEFAULT_PANEL_ID].mode;
#endif //USED_FW		
		break;
	default:
				    ...
				    size_t nbytes,loff_t *ppos)
{
	...
	struct drm_minor *minor = (struct drm_minor *)PDE(file_inode(file))->data;
#else
...
#ifdef CONFIG_BOARD_CTP_SAMSUNG
	tc35876x_lvds_bridge_init();
	psb_backlight_sec_init();
	fcall_cmc624_init();
#endif
